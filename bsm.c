...
// The LIV components are defined in the Sun-centered celestial equatorial 
// frame 
// Barenboim et al. PLB 788, 308 (2019)
// ----------------------------------------------------------------------------
// Author: Andre Steklain (steklain@utfpr.edu.br)
// Based on snu.c by Joachim Kopp (jkopp@fnal.gov)
// ----------------------------------------------------------------------------
...
#define MAX_PARAMS    (8*SQR(MAX_FLAVORS) - MAX_FLAVORS)
...
static double complex a_v[MAX_FLAVORS][MAX_FLAVORS];        // LIV (a)
static double complex c_v[MAX_FLAVORS][MAX_FLAVORS];        // LIV (c)

// Names of BSM parameters
char bsm_param_strings[MAX_PARAMS][64];
...
// ----------------------------------------------------------------------------
// Initialize probability engine for the 3-flavor case with BSM (no steriles)
// ----------------------------------------------------------------------------
{
  int rotation_order[][2] = { {2,3}, {1,3}, {1,2} };
  int phase_order[] = { -1, 0, -1 };
  return bsm_init_probability_engine(3, rotation_order, phase_order);
}
...
// ----------------------------------------------------------------------------
{
  ...
  {
    fprintf(stderr, "bsm_init_probability_engine: Too many or too few neutrino flavors (%d).\n",
            _n_flavors);
    return -1;
  }
  ...
  //   n^2 |\eps^d|, n^2 \phi^d
  //   n (n+1)/2 |a^v|, n(n-1)/2 \phia^v,
  //   n (n+1)/2 |c^v|, n(n-1)/2 \phic^v,
  // = 8 n^2 - n
  n_flavors = _n_flavors;
  n_params  = 8*SQR(n_flavors) - n_flavors;
  ...
  for (int i=0; i < n_angles; i++)
  {
    ...
    {
      fprintf(stderr, "bsm_init_probability_engine: Incorrect rotation order specification.\n");
      return -2;
    }
    if (_phase_order[i] >= n_phases)
    {
      fprintf(stderr, "bsm_init_probability_engine: Incorrect phase order specification.\n");
      return -3;
    }
...
//  printf("\n");
  ...
  sprintf(bsm_param_strings[0], "%s", "TH12");    // Standard oscillation parameters
  sprintf(bsm_param_strings[1], "%s", "TH13");
  sprintf(bsm_param_strings[2], "%s", "TH23");
  sprintf(bsm_param_strings[3], "%s", "DELTA_0");
  sprintf(bsm_param_strings[4], "%s", "DM21");
  sprintf(bsm_param_strings[5], "%s", "DM31");

  int k = 6;
  for (int i=4; i <= n_flavors; i++)            // Mass squared differences
    sprintf(bsm_param_strings[k++], "DM%d1", i);

  for (int i=1; i <= n_flavors; i++)            // Sterile mixing angles
    for (int j=MAX(i+1,4); j <= n_flavors; j++)
      sprintf(bsm_param_strings[k++], "TH%d%d", i, j);

  for (int i=1; i <= n_phases-1; i++)
    sprintf(bsm_param_strings[k++], "DELTA_%d", i); // Sterile phases

  const char *flavors[] = { "E", "MU", "TAU", "S1", "S2", "S3", "S4", "S5", "S6" };
  for (int i=0; i < n_flavors; i++)             // Source NSI
    for (int j=0; j < n_flavors; j++)
    {
      sprintf(bsm_param_strings[k++], "ABS_EPS_S_%s%s", flavors[j], flavors[i]);
      sprintf(bsm_param_strings[k++], "ARG_EPS_S_%s%s", flavors[j], flavors[i]);
    }

  for (int i=0; i < n_flavors; i++)             // Propagation NSI
  {
    sprintf(bsm_param_strings[k++], "EPS_M_%s%s", flavors[i], flavors[i]);
    for (int j=i+1; j < n_flavors; j++)
    {
      sprintf(bsm_param_strings[k++], "ABS_EPS_M_%s%s", flavors[i], flavors[j]);
      sprintf(bsm_param_strings[k++], "ARG_EPS_M_%s%s", flavors[i], flavors[j]);
    }
  }
  ...
  for (int i=0; i < n_flavors; i++)             // Detector NSI
    for (int j=0; j < n_flavors; j++)
    {
      sprintf(bsm_param_strings[k++], "ABS_EPS_D_%s%s", flavors[j], flavors[i]);
      sprintf(bsm_param_strings[k++], "ARG_EPS_D_%s%s", flavors[j], flavors[i]);
    }


  for (int i=0; i < n_flavors; i++)             // LIV (a)
  {
    sprintf(bsm_param_strings[k++], "A_V_%s%s", flavors[i], flavors[i]);
    for (int j=i+1; j < n_flavors; j++)
    {
      sprintf(bsm_param_strings[k++], "ABS_A_V_%s%s", flavors[i], flavors[j]);
      sprintf(bsm_param_strings[k++], "ARG_A_V_%s%s", flavors[i], flavors[j]);
    }
  }
    
  for (int i=0; i < n_flavors; i++)             // LIV (c)
  {
    sprintf(bsm_param_strings[k++], "C_V_%s%s", flavors[i], flavors[i]);
    for (int j=i+1; j < n_flavors; j++)
    {
      sprintf(bsm_param_strings[k++], "ABS_C_V_%s%s", flavors[i], flavors[j]);
      sprintf(bsm_param_strings[k++], "ARG_C_V_%s%s", flavors[i], flavors[j]);
    }
  }

  if (k != n_params)
  {
    fprintf(stderr, "bsm_init_probability_engine: n_params has an incorrect value (%d).\n",
            n_params);
...
//  {
//    printf("  %-20s", bsm_param_strings[i]);
//    if (i % 4 == 3)  printf("\n");
//  }
...
//      glbSetDensityParams(p, glbGetDensityParams(p, density_corr[j]), j);
  ...
  for (i=0; i < n_flavors; i++)                 // LIV (a)
  {
    a_v[i][i] = glbGetOscParams(p,k);
    k++;
    for (j=i+1; j < n_flavors; j++)
    {
      a_v[i][j] = glbGetOscParams(p,k) * cexp(I*glbGetOscParams(p,k+1));
      a_v[j][i] = conj(a_v[i][j]);
      k += 2;
    }
  }
  
  for (i=0; i < n_flavors; i++)                 // LIV (c)
  {
    c_v[i][i] = glbGetOscParams(p,k);
    k++;
    for (j=i+1; j < n_flavors; j++)
    {
      c_v[i][j] = glbGetOscParams(p,k) * cexp(I*glbGetOscParams(p,k+1));
      c_v[j][i] = conj(c_v[i][j]);
      k += 2;
    }
  }
...
// ----------------------------------------------------------------------------
{
  ...
  for (i=0; i < n_flavors; i++)                 // Propagation LIV (a)
  {
    glbSetOscParams(p, a_v[i][i], k);
    k++;
    for (j=i+1; j < n_flavors; j++)
    {
      glbSetOscParams(p, cabs(a_v[i][j]), k);
      glbSetOscParams(p, carg(a_v[i][j]), k+1);
      k += 2;
    }
  }

  for (i=0; i < n_flavors; i++)                 // Propagation LIV (c)
  {
    glbSetOscParams(p, c_v[i][i], k);
    k++;
    for (j=i+1; j < n_flavors; j++)
    {
      glbSetOscParams(p, cabs(c_v[i][j]), k);
      glbSetOscParams(p, carg(c_v[i][j]), k+1);
      k += 2;
    }
  }
...
// ----------------------------------------------------------------------------
int bsm_hamiltonian_cd(double E, double rho, int cp_sign)
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
{
  ...
  if (cp_sign > 0)
  {
    for (i=0; i < n_flavors; i++)
      for (j=0; j < n_flavors; j++)
        _H[i][j] = _H0_template[i][j] * inv_E  +  Ve*epsilon_m[i][j] + a_v[i][j] - ftE * c_v[i][j];
  }
  else
  {
    for (i=0; i < n_flavors; i++)
...
// ----------------------------------------------------------------------------
int bsm_S_matrix_cd(double E, double L, double rho, int cp_sign)
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
{
  ...
  else                                             // Matter
  {
    ...
    if ((status=bsm_hamiltonian_cd(E, rho, cp_sign)) != 0)
      return status;
...
// ----------------------------------------------------------------------------
{
  ...
  status = bsm_probability_matrix_all(P, cp_sign, E, psteps, length, density,
...
// ----------------------------------------------------------------------------
{
  ...
  {
    if (psteps == 1)
      bsm_filtered_probability_matrix_cd(P, E, GLB_KM_TO_EV(length[0]),
  ...
  else                                        // Without low-pass filter
  {
    if (psteps > 1)
    {
      ...
      for (i=0; i < psteps; i++)
      {
        status = bsm_S_matrix_cd(E, GLB_KM_TO_EV(length[i]), density[i], cp_sign);
        if (status != 0)
          return status;
    ...
    {
      status = bsm_S_matrix_cd(E, GLB_KM_TO_EV(length[0]), density[0], cp_sign);
      if (status != 0)
        return status;
    }
